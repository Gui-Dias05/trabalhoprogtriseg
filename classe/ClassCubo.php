<?php
    ...
    class Cubo extends Quadrado{
        ...
        public function inseri(){
            $sql = 'INSERT INTO trabalho.cubo (cor, idquadrado) 
        ...
        public function exclui(){
            $sql = 'DELETE FROM trabalho.cubo WHERE idcubo = :idcubo';
            $parametros = array(":idcubo"=>$this->getidcubo());
            return parent::executaComando($sql,$parametros);
        }

        public function edita(){
            $sql = 'UPDATE trabalho.cubo 
        ...
        public static function listar($buscar = 0, $procurar = ""){
            $sql = "SELECT * FROM quadrado,cubo WHERE idquadrado=id";
            if ($buscar > 0)
                switch($buscar){
                    case(1): $sql .= " AND idcubo like :procurar"; $procurar = "%".$procurar."%"; break;
                    case(2): $sql .= " AND cor like :procurar"; $procurar = "%".$procurar."%"; break;
                    case(3): $sql .= " AND idquadrado like :procurar"; $procurar = "%".$procurar."%"; break;
                }
            if ($buscar > 0)
